<!-- source: https://tailscale.com/kb/1103/exit-nodes pulled 2026-07-04 -->
...
Use the following steps to configure an exit node:
  ...
  4. Grant access to use the exit node.
  5. Use the exit node.
...
This changes if you customize your tailnet's [access control policy](/docs/features/access-control/acls). For example, suppose you replace the default policy with one that only lets your developers access a few internal servers:
...
Your developers can still reach the production servers, but they can no longer use an exit node. The policy never grants access to [`autogroup:internet`](/docs/reference/examples/grants#allow-using-exit-nodes), so Tailscale has no permission to route internet traffic through the exit node.
...
To permit exit node use, add a [grant](/docs/features/access-control/grants) or ACL whose `dst` is `autogroup:internet`. This grants permission to route internet traffic through any configured exit node rather than to connect to the exit node device itself.
...
Destination Logging is available for [the Premium and Enterprise plans](/pricing). This feature requires a sales contract to enable, contact [Tailscale Sales](/contact/sales).
